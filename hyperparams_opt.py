...
def sample_sac_params(trial: optuna.Trial) -> Dict[str, Any]:
    ...
    gamma = trial.suggest_categorical(
        "gamma",
        [0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.98, 0.99, 0.995, 0.999, 0.9999],
    )
...
def objective(trial: optuna.Trial) -> float:
    marl_comm_env, eval_env, _ = env_creator(env_config)
    kwargs = DEFAULT_HYPERPARAMS.copy()
    kwargs.update(sample_sac_params(trial))
    model = SAC(env=marl_comm_env, seed=SEED, **kwargs)  # type: ignore
...
if __name__ == "__main__":
    ...
    joblib.dump(study, "./agents/models/hyperparameter_opt/sb3_sac.pkl")
